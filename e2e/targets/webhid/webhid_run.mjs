// Playwright driver for the WebHID conformance harness: serves index.html + the
...
function startServer() {
  ...
  // Fixed port so the WebHidAllowDevicesForUrls policy origin matches.
...
async function main() {
  ...
  const browser = await chromium.launch({
    ...
    headless: false, // WebHID needs full Chrome (run under Xvfb)
    args: ["--no-sandbox"],
  });
  ...
  // The WebHidAllowDevicesForUrls policy (see run.sh) pre-grants the fixture, so
  // get_devices() returns it without a chooser.
  await page.goto(url);
  ...
  if (result && result.ok) {
    console.log("WEBHID_RESULT_OK", result.summary);
    process.exit(0);
  } else {
    console.error("WEBHID_RESULT_FAIL", result && result.error);
    process.exit(1);
  }
}
